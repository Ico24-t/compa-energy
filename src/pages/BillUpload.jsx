...
import { createLead, saveConsumption, markPreContractAsSent, updateLeadStatus, saveOfferCalculation, createPreContract } from '../services/api'
...
const BillUpload = () => {
  ...
  const onDrop = useCallback(async (files) => {
    ...
    try {
      ...
      if (!risposta.ok) throw new Error(`Worker error: ${risposta.status}`)

      const risultato = await risposta.json()
      if (!risultato.success || !risultato.dati) throw new Error('Dati non estratti')

      const dati = risultato.dati
      if (!dati.importo_totale) { setFase(FASE.NON_LEGGIBILE); return }

      // Annualizza consumi se presenti
      const mesi = dati.mesi_periodo || 2
      if (dati.consumo_kwh > 0) dati.consumo_annuo_kwh = annualizzaConsumi(dati.consumo_kwh, mesi)
      if (dati.consumo_smc > 0) dati.consumo_annuo_smc = annualizzaConsumi(dati.consumo_smc, mesi)
      ...
      if (dati.intestatario) {
        ...
        if (parti.length >= 2) { setCognome(parti[0]); setNome(parti.slice(1).join(' ')) }
      }
  ...
  const handleContatti = async () => {
    ...
    try {
      ...
      const leadRes = await createLead({
        email, telefono: formatPhone(telefono),
        tipo_cliente: 'privato', origine: 'upload_bolletta',
        privacy_acconsentito: true, ip_address: ipAddress
      })
      ...
      await saveConsumption({
        lead_id: lid, tipo_fornitura: tipoFornitura,
        consumo_annuo_kwh: consumoKwh, consumo_annuo_smc: consumoSmc,
      ...
      const best = findBestOffer(offersRes.data, { consumo_annuo_kwh: consumoKwh, consumo_annuo_smc: consumoSmc }, currentCost, 'privato')
      ...
      await saveOfferCalculation({
        lead_id: lid, offerta_id: best.id,
    ...
    <div className="container mx-auto max-w-2xl px-4 py-8">
      ...
      <AnimatePresence mode="wait">
        ...
        {fase === FASE.UPLOAD && (
          <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="text-center mb-8">
              ...
              <p className="text-slate-600">Analizziamo la bolletta e calcoliamo il risparmio in pochi secondi</p>
            </div>
            <div {...getRootProps()}
        ...
        {fase === FASE.ELABORAZIONE && (
          <motion.div key="elab" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="card text-center py-16">
        ...
        {fase === FASE.NON_LEGGIBILE && (
          <motion.div key="nonlegg" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="card border-orange-200 bg-orange-50 text-center py-10">
              ...
              <p className="text-slate-600 mb-2">Non siamo riusciti a leggere tutti i dati necessari.</p>
              <p className="text-slate-500 text-sm mb-8">Prova con una foto più nitida oppure inserisci i dati manualmente.</p>
          ...
          <motion.div key="contatti" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="card mb-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-xl font-bold text-slate-900">Dati letti dalla bolletta</h3>
              </div>
              <div className="space-y-2 text-sm">
                {datiEstratti.intestatario && <Row label="Intestatario" value={datiEstratti.intestatario} />}
                {datiEstratti.tipo_fornitura && <Row label="Tipo fornitura" value={datiEstratti.tipo_fornitura.toUpperCase()} />}
                {datiEstratti.importo_totale && <Row label="Importo totale" value={`€ ${Number(datiEstratti.importo_totale).toFixed(2)}`} />}
                {datiEstratti.data_inizio && <Row label="Periodo" value={`${datiEstratti.data_inizio} – ${datiEstratti.data_fine}`} />}
                {datiEstratti.consumo_kwh > 0 && <Row label="Consumo periodo" value={`${datiEstratti.consumo_kwh} ${datiEstratti.tipo_fornitura === 'gas' ? 'Smc' : 'kWh'}`} />}
                {datiEstratti.consumo_annuo_kwh > 0 && <Row label="→ Annuo luce" value={`${datiEstratti.consumo_annuo_kwh} kWh/anno`} />}
                {datiEstratti.consumo_annuo_smc > 0 && <Row label="→ Annuo gas" value={`${datiEstratti.consumo_annuo_smc} Smc/anno`} />}
                {datiEstratti.fornitore && <Row label="Fornitore attuale" value={datiEstratti.fornitore} />}
                {datiEstratti.indirizzo_fornitura && <Row label="Indirizzo" value={`${datiEstratti.indirizzo_fornitura}, ${datiEstratti.cap || ''} ${datiEstratti.citta || ''}`} />}
            ...
            <div className="card">
              <h3 className="text-lg font-bold text-slate-900 mb-4">I tuoi dati</h3>
        ...
        {fase === FASE.OFFERTA_BUONA && (
          <motion.div key="buona" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="card text-center">
            ...
            <p className="text-slate-600 mb-6">Al momento non abbiamo offerte più convenienti. Ti avviseremo quando ne avremo una migliore.</p>
        ...
        {fase === FASE.RINGRAZIAMENTO && (
          <motion.div key="grazie" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="card text-center">
            ...
            <p className="text-sm text-slate-500">Per info: <strong>{import.meta.env.VITE_OPERATOR_PHONE}</strong></p>
          </motion.div>
        )}
