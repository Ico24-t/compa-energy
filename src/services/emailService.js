...
export const inviaEmailCliente = async (datiCliente, offerta, preContratto) => {
  try {
    const templateParams = {
      ...
      // Dati cliente - CORREZIONE QUI
      codice_pratica: preContratto && preContratto.id ? preContratto.id.substring(0, 8).toUpperCase() : 'PRATICA_' + Date.now(),
...
export const inviaEmailOperatore = async (datiCliente, offerta, preContratto, leadId) => {
  try {
    const templateParams = {
      ...
      // Info sistema
      lead_id: leadId,
      pre_contratto_id: preContratto ? preContratto.id : 'N/D'
    };
